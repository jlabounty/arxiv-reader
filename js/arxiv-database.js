/* arxiv-database.js — full arXiv category tree (global scope) */
/* Ported from the original dailyarxiv arxiv-selector.js by juanjosegarciaripoll */

/**
 * Each top-level entry is a discipline group.
 * Each group has subcategories (areas).
 * Some areas have their own subcategories (sub-areas).
 *
 * Structure:
...
/**
 * Returns all leaf category IDs (the ones we actually query the arXiv API with)
 * for a given area entry. If the area has no subcategories, returns [area.id].
 */
function getLeafIds(area) {
  if (!area.subcategories || area.subcategories.length === 0) {
    return [area.id];
  }
  return area.subcategories.map(s => s.id);
}
